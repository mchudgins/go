...
func (h *hystrixHelper) Reset() {
	h.logger.Warn("HystrixHelper reset called -- not implemented yet")
}

func (h *hystrixHelper) Update(mr metricCollector.MetricResult) {
	l := prometheus.Labels{"circuit": h.commandName}

	hystrixAttempts.With(l).Add(mr.Attempts)
	hystrixErrors.With(l).Add(mr.Errors)
	hystrixSuccesses.With(l).Add(mr.Successes)
	hystrixFailures.With(l).Add(mr.Failures)
	hystrixRejects.With(l).Add(mr.Rejects)
	hystrixShortCircuits.With(l).Add(mr.ShortCircuits)
	hystrixTimeouts.With(l).Add(mr.Timeouts)
	hystrixFallbackSuccesses.With(l).Add(mr.FallbackSuccesses)
	hystrixFallbackFailures.With(l).Add(mr.FallbackFailures)
	hystrixTotalDuration.With(l).Add(float64(mr.TotalDuration.Microseconds()))
	hystrixRunDuration.With(l).Add(float64(mr.RunDuration.Microseconds()))

	h.logger.Debug("HystrixHelper.Update called")
}
